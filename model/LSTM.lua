...
function layer:__init(input_dim, hidden_dim)
  ...
  -- self.initiailzed_before = false
  self.set_c0_h0_manually = false
  ...
  self.cell = torch.Tensor()    -- cur_sz = 0, This will be (N, T, H)
  self.gates = torch.Tensor()   -- cur_sz = 0, This will be (N, T, 4H)
  self.buffer1 = torch.Tensor() -- cur_sz = 0, This will be (N, H)
  self.buffer2 = torch.Tensor() -- cur_sz = 0, This will be (N, H)
  self.buffer3 = torch.Tensor() -- cur_sz = 0, This will be (1, 4H)
  self.grad_a_buffer = torch.Tensor() -- cur_sz = 0, This will be (N, 4H)

  self.batch_len = torch.Tensor() -- zero/non-zero err test This should be manually feed in by outside
  ...
  self.grad_c0 = torch.Tensor() -- cur_sz = 0,
  self.grad_h0 = torch.Tensor() -- cur_sz = 0,
  self.grad_x = torch.Tensor() -- cur_sz = 0,
...
function layer:reset(std)
  ...
  self.bias:zero()
--  self.bias[{{self.hidden_dim + 1, 2 * self.hidden_dim}}]:fill(1)  -- forget gate bias starts from large positive value
...
function layer:resetStates()
  self.h0 = self.h0.new()
  self.c0 = self.c0.new()
  print('LSTM : resetStates')
end

local function check_dims(x, dims)
  assert(x:dim() == #dims)
  for i, d in ipairs(dims) do
    --print('x:size(i) = '..x:size(i)..', d = '..d)
    assert(x:size(i) == d)
  end
end

local function check_two_vars_same_dims(x, y)
  assert(x:dim() == y:dim())
  for i, d in ipairs(x:size()) do
...
function layer:_unpack_input(input)
  ...
  --[[
  if(self.initiailzed_before) then
    c0 = self.c0
    h0 = self.h0
    self.initiailzed_before = false
  end
...
function layer:updateOutput(input)
  self.recompute_backward = true   -- Q) 무슨 옵션? BPTT는 똑같은 모듈에서 계속 backprop을 해야하기 때문에, 같은 모듈내에서 backprop을 유지하도록 만들어주는 것인듯?
  local c0, h0, x = self:_unpack_input(input)
  ...
  if not c0 then
    c0 = self.c0
    if c0:nElement() == 0 or (not self.remember_states and not self.set_c0_h0_manually) then
      c0:resize(N, H):zero()
      -- elseif (self.remember_states and self.set_c0_h0_manually) then
  ...
  if not h0 then
    h0 = self.h0
    if h0:nElement() == 0 or (not self.remember_states and not self.set_c0_h0_manually) then
      h0:resize(N, H):zero()
      -- elseif (self.remember_states and self.set_c0_h0_manually) then
    elseif self.remember_states then
  ...
  local bias_expand = self.bias:view(1, 4 * H):expand(N, 4 * H)   -- N개의 sequence에 대해 똑같은 bias vector를 더해줌
  ...
  for t = 1, T do
    ...
    -- If you are interested in batch normalization over Wx, see LSTM_batchnormWx_fail.lua
...
function layer:backward(input, gradOutput, scale)
  ...
  -- check_two_vars_same_dims(h, gradOutput)
  ...
  for t = T, 1, -1 do
    ...
    -- Gradients for block & cell
    --for batchIdx = 1, N do
--      if(self.batch_len[batchIdx] == t) then
        --print('(zero err) t = '..t..', batchIdx = '..batchIdx..', grad_next_h[ {{batchIdx}, {}}]:norm() = '..grad_next_h[ {{batchIdx}, {}}]:norm())
        --print('(zero err)t = '..t..', batchIdx = '..batchIdx..', grad_next_c[ {{batchIdx}, {}}]:norm() = '..grad_next_c[ {{batchIdx}, {}}]:norm())
      --end
    --end

    -- Gradients for output/input (move to Bottom)
    ...
    -- Gradients for output/input 
    --for batchIdx = 1, N do
--      if( (self.batch_len[batchIdx] + 1 )== t) then
        --print('(zero err)t+1 = '..t..', batchIdx = '..batchIdx..', grad_x[ {{batchIdx}, {}}]:norm() = '..self.grad_x[ {{batchIdx}, {}}]:norm())
        --print('(zero err)t+1 = '..t..', batchIdx = '..batchIdx..', gradOutput[ {{batchIdx}, {t}, {}}]:norm() = '..gradOutput[ {{batchIdx}, {t}, {}}]:norm())
      --end
    --end
  end
